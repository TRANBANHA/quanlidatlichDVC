<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        // Cập nhật ENUM để xóa qr_code và thêm vnpay
        // Bước 1: Mở rộng ENUM tạm thời để chứa tất cả giá trị có thể
        DB::statement("ALTER TABLE thanh_toan MODIFY COLUMN phuong_thuc_thanh_toan ENUM('qr_code', 'tien_mat', 'chuyen_khoan', 'vnpay', 'zalopay') DEFAULT 'tien_mat'");
        
        // Bước 2: Cập nhật các payment có phuong_thuc_thanh_toan = 'qr_code' thành 'vnpay'
        DB::table('thanh_toan')
            ->where('phuong_thuc_thanh_toan', 'qr_code')
            ->update(['phuong_thuc_thanh_toan' => 'vnpay']);
        
        // Bước 3: Thu hẹp ENUM về chỉ các giá trị hợp lệ (loại bỏ qr_code)
        DB::statement("ALTER TABLE thanh_toan MODIFY COLUMN phuong_thuc_thanh_toan ENUM('vnpay', 'tien_mat', 'chuyen_khoan') DEFAULT 'tien_mat'");
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        // Khôi phục lại qr_code
        DB::statement("ALTER TABLE thanh_toan MODIFY COLUMN phuong_thuc_thanh_toan ENUM('qr_code', 'tien_mat', 'chuyen_khoan', 'vnpay', 'zalopay') DEFAULT 'tien_mat'");
        
        // Cập nhật lại các payment có vnpay về qr_code (nếu cần)
        // DB::table('thanh_toan')
        //     ->where('phuong_thuc_thanh_toan', 'vnpay')
        //     ->update(['phuong_thuc_thanh_toan' => 'qr_code']);
        
        // DB::statement("ALTER TABLE thanh_toan MODIFY COLUMN phuong_thuc_thanh_toan ENUM('qr_code', 'tien_mat', 'chuyen_khoan') DEFAULT 'tien_mat'");
    }
};

